...
const General = props => {
    ...
    const { data: exists, isLoading: isExisitsLoading } = useQuery('exists', async () => {
        const result = await fetch(`${process.env.REACT_APP_API}/api/request/user?id=${keycloak.idTokenParsed.sub}&type=3`)
    ...
    const schema = yup.object().shape({
        ...
        hasLostJobDueCorona: yup.bool().nullable().required('Por fabor indiká si bo a pèrdè trabou pa motibu di Covid'),
        //proofOfLostJobDueCovid: yup.array().required('Por fabor ‘upload’ prueba di a pèrdè trabou pa motibu di Covid'),
        monthlyRedCrossDonation: yup.string().required('Por fabor yena e montante mensual ku bo tabata risibí na Krus Kòra'),
        proofOfRedCrossCard: yup.array().required('Mester ‘upload’ prueba di e karchi di Krus Kòrá, saka un potrèt di e karchi'),
                                ...
                                <Controller
                                    ...
                                    render={({ onChange, value }) => (
                                        <>
                                            ...
                                            <RadioGroup aria-label="contractee" name="contractee" value={value} onChange={onChange}>
                                                ...
                                                <FormControlLabel value="pensionLessThan500" control={<Radio />} label="Penshun mènos 500" />
                                ...
                                <Controller
                                    ...
                                    render={({ onChange, value }) => (
                                        <>
                                            ...
                                            <RadioGroup aria-label="hasLostJobDueCorona" name="hasLostJobDueCorona" value={JSON.parse(value)} onChange={onChange} row>
                                                <FormControlLabel value={true} control={<Radio />} label="Si" />
                                                <FormControlLabel value={false} control={<Radio />} label="Nò" />
                                            </RadioGroup>
                                        </>
                                ...
                                <Controller
                                    ...
                                    render={({ onChange, value }) => (
                                        <>
                                            ...
                                            <RadioGroup aria-label="monthlyRedCrossDonation" name="monthlyRedCrossDonation" value={value} onChange={onChange}>
